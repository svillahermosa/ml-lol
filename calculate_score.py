...
df = pd.read_csv(r'C:\DevelopmentLaLiga\ml-lol\extracts\full_high_elo.tsv', sep='\t')

df['dt_fecha'] = df.apply(lambda x: datetime.fromtimestamp(x.ts_fecha/1000).strftime('%d-%m-%y'), axis=1)
df2 = df[['dt_fecha', 'jugador', 'pk_partida']]
df3 = df[['jugador', 'pk_partida', 'lane']]


df_groupby = df3.groupby(['jugador', 'lane']).size().reset_index(name='partidas_por_posicion')
...
df_merged_1 = pd.merge(df, df_percentage_lane, left_on=['jugador'], right_on=['jugador'], how='inner')
...
df_merged_2.to_csv(r'C:\DevelopmentLaLiga\ml-lol\extracts\full_high_elo.tsv', sep='\t')
